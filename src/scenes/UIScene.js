  ...
  _drawEquipmentPanel() {
    ...
    trashRarities.forEach((tr, i) => {
      ...
      const tLabel = this.add.text(tx + trashBtnW / 2, trashY + trashBtnH / 2, tr.label, {
        fontFamily: 'monospace', fontSize: '9px', color: tr.color,
      ...
      for (let col = 0; col < INVENTORY_COLS; col++) {
        ...
        if (item) {
          ...
          const arrow = this.add.text(cx + cellW - 4, cy + 2, arrowText, {
            fontFamily: 'monospace', fontSize: '9px', color: arrowColor, fontStyle: 'bold',
          }).setOrigin(1, 0).setDepth(3);
  ...
  _showEquipTooltip(item, x, y) {
    ...
    lines.forEach((line, i) => {
      container.add(this.add.text(ttX + 8, ttY + 8 + i * 18, line, {
        fontFamily: 'monospace', fontSize: '10px', color: '#ccddee',
      }));
    });
  }
  ...
  _onPauseToggled(isPaused) {
    ...
    if (isPaused) {
      this._pauseIndicator = this.add.text(UI_X + UI_W / 2, 2, '\u23F8 PAUSED - Manage your gear!', {
        fontFamily: 'monospace', fontSize: '9px', color: '#4ade80', fontStyle: 'bold',
