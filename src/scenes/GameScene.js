...
export class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(talentBtnX, talentBtnY, 'T: Talents', {
      fontFamily: 'monospace', fontSize: '14px', color: '#f0c040',
  ...
  _showAbilityTooltip(x, y, def) {
    ...
    container.add(this.add.text(ttX + 8, ttY + 24, def.desc, {
      fontFamily: 'monospace', fontSize: '14px', color: '#ccccdd', lineSpacing: 2,
    }));
  ...
  _showTalentTree() {
    ...
    container.add(this.add.text(w / 2, py + 42, `Available Points: ${available}  (1 per level)`, {
      fontFamily: 'monospace', fontSize: '13px', color: available > 0 ? '#4ade80' : '#888899',
    }).setOrigin(0.5));
    const trees = Object.entries(this.classData.talents);
    const treeW = 260, treeGap = 20;
    ...
    trees.forEach(([, tree], treeIdx) => {
      ...
      container.add(this.add.text(tx + treeW / 2, treeY, `${tree.icon} ${tree.name}`, {
        fontFamily: 'monospace', fontSize: '14px', color: tree.color, fontStyle: 'bold',
      }).setOrigin(0.5));
      container.add(this.add.text(tx + treeW / 2, treeY + 18, tree.description, {
        fontFamily: 'monospace', fontSize: '10px', color: '#888899', wordWrap: { width: treeW - 10 },
      }).setOrigin(0.5, 0));
      ...
      tree.tiers.forEach((talent, tierIdx) => {
        ...
        container.add(this.add.text(tx + treeW / 2, ny + 8, talent.name, {
          fontFamily: 'monospace', fontSize: '12px', color: allocated > 0 ? tree.color : '#aaaacc', fontStyle: 'bold',
        }).setOrigin(0.5));
        container.add(this.add.text(tx + treeW / 2, ny + 22, talent.description, {
          fontFamily: 'monospace', fontSize: '9px', color: '#888899', wordWrap: { width: treeW - 40 },
        }).setOrigin(0.5, 0));
        container.add(this.add.text(tx + treeW - 16, ny + 4, `${allocated}/${talent.maxPoints}`, {
          fontFamily: 'monospace', fontSize: '10px', color: allocated > 0 ? '#4ade80' : '#555566',
        }).setOrigin(1, 0));
        if (canAllocate) {
